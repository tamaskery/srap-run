...
const context=await browser.newContext({viewport:{width:1920,height:1080},recordVideo:{dir:'evidence/g5/video',size:{width:1920,height:1080}}});
...
try{
 ...
 await page.evaluate(()=>{const m=window.__m0;m.place('player',{x:-30,y:0,z:-10});m.command({x:-30,y:0,z:14});m.action({type:'move',x:0,z:0,sprint:false});});
 await page.waitForTimeout(2200);await page.screenshot({path:'evidence/g5/pavilion-travel.png'});
 await page.waitForTimeout(2000);await page.screenshot({path:'evidence/g5/pavilion-cutaway.png'});
 await page.evaluate(()=>window.__m0.pause(true));await page.waitForTimeout(600);
 ...
 await page.waitForTimeout(200);await page.screenshot({path:'evidence/g5/actors-close.png'});
}finally{
 await context.close();await page.video().saveAs('evidence/g5/slice-motion.webm');
