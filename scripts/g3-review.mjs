...
const definitions={};
try{
 for(const [name,path,port] of [['before','evidence/g3/g2-dist',4191],['slice','dist',4190]]){
  ...
  for(const [label,span] of [['normal',64],['wide',76],['detail',32]]){
   ...
   await page.waitForTimeout(250);await page.screenshot({path:`evidence/g3/${name}-${label}.png`});
   if(name==='slice'&&label==='normal'){
    ...
    await page.screenshot({path:'evidence/g3/proxy-overlay.png'});
    await page.evaluate(()=>{document.querySelector('#proxy-review').remove();document.body.style.filter='grayscale(1)';});
    await page.screenshot({path:'evidence/g3/slice-greyscale.png'});await page.evaluate(()=>document.body.style.filter='');
   }
  }
  if(name==='slice'){
   ...
   await page.screenshot({path:'evidence/g3/slice-cutaway.png'});
   await page.setViewportSize({width:1366,height:768});await page.screenshot({path:'evidence/g3/slice-1366.png'});
   await writeFile('evidence/g3/snapshot.json',JSON.stringify(await page.evaluate(()=>window.__m0.snapshot()),null,2));
  }
  await page.close();
 }
 await writeFile('evidence/g3/capture-errors.json',JSON.stringify(errors));
 await writeFile('evidence/g3/definition-comparison.json',JSON.stringify({beforeEqualsSlice:JSON.stringify(definitions.before)===JSON.stringify(definitions.slice)},null,2));
