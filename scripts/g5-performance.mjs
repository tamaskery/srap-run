...
const baseline=await preview({configFile:false,build:{outDir:'evidence/g5/g4-dist'},preview:{host:'127.0.0.1',port:4185,strictPort:true}});
...
const output=process.env.G5_OUTPUT??'evidence/g5';
try{
 for(const [label,port] of [['g4-1',4185],['g5-1',4187],['g4-2',4185],['g5-2',4187]]){
  ...
  for(const state of (process.env.G2_PHASES?.split(',')??['idle','walk','sprint','court'])){
   ...
   await writeFile(output+'/performance-final.json',JSON.stringify({browser:browser.version(),headless:true,startup,results},null,2));
 ...
 await writeFile(output+'/replay-performance.json',JSON.stringify(replays,null,2));
