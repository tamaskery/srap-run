...
const definitions={};
try{
 for(const [name,path,port] of (process.env.CANDIDATE_ONLY?[['slice','dist',4190]]:[['before','evidence/g5/g4-dist',4191],['slice','dist',4190]])){
  ...
  page.on('pageerror',e=>{errors.push({name,error:e.message});console.error(e.message);});
  await page.goto(`http://127.0.0.1:${port}/?debug&scene=square&deterministic`);
  await page.waitForFunction(()=>window.__m0,null,{timeout:60000}).catch(async e=>{console.error(await page.locator('body').innerText());throw e;});
  ...
  for(const [label,span] of [['normal',64],['wide',76],['detail',32]]){
   ...
   await page.waitForTimeout(250);await page.screenshot({path:`evidence/g5/${name}-${label}.png`});
   if(name==='slice'&&label==='normal'){
    ...
    await page.screenshot({path:'evidence/g5/proxy-overlay.png'});
    await page.evaluate(()=>{document.querySelector('#proxy-review').remove();document.body.style.filter='grayscale(1)';});
    await page.screenshot({path:'evidence/g5/slice-greyscale.png'});await page.evaluate(()=>document.body.style.filter='');
   }
  }
  if(name==='slice'){
   ...
   await page.screenshot({path:'evidence/g5/slice-cutaway.png'});
   await page.setViewportSize({width:1366,height:768});await page.screenshot({path:'evidence/g5/slice-1366.png'});
   await writeFile('evidence/g5/snapshot.json',JSON.stringify(await page.evaluate(()=>window.__m0.snapshot()),null,2));
  }
  ...
  await page.screenshot({path:`evidence/g5/${name}-pavilions.png`});
  await page.close();
 }
 await writeFile('evidence/g5/capture-errors.json',JSON.stringify(errors));
 await writeFile('evidence/g5/definition-comparison.json',JSON.stringify({beforeEqualsSlice:JSON.stringify(definitions.before)===JSON.stringify(definitions.slice)},null,2));
