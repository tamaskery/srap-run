...
const baseline=await preview({configFile:false,build:{outDir:'evidence/g1/baseline-dist'},preview:{host:'127.0.0.1',port:4185,strictPort:true}});
...
const results=[];
try{
 for(const [label,port] of [['baseline-1',4185],['hero-1',4187],['baseline-2',4185],['hero-2',4187]]){
  ...
  for(const state of ['idle','walk','sprint']){
   ...
   for(let i=0;i<cycles;i++){
    await page.evaluate(state=>{const m=window.__m0;m.place('player',{x:-32,y:0,z:-13});if(state!=='idle')m.command({x:-32,y:0,z:10});m.action({type:'move',x:0,z:0,sprint:state==='sprint'});},state);
   ...
   await writeFile('evidence/g1/performance.json',JSON.stringify({browser:browser.version(),headless:true,results},null,2));
