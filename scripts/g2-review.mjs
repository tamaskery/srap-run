...
const definitions={};
try{
 for(const [name,path,port] of [['control','evidence/g2/control-dist',4191],['slice','dist',4190],['m4','evidence/g1/baseline-dist',4192]]){
  ...
  for(const [label,span] of [['normal',64],['wide',76],['detail',32]]){
   ...
   await page.waitForTimeout(250);await page.screenshot({path:`evidence/g2/${name}-${label}.png`});
   if(name==='slice'&&label==='normal'){
    ...
    await page.screenshot({path:'evidence/g2/proxy-overlay.png'});
    await page.evaluate(()=>{document.querySelector('#proxy-review').remove();document.body.style.filter='grayscale(1)';});
    await page.screenshot({path:'evidence/g2/slice-greyscale.png'});await page.evaluate(()=>document.body.style.filter='');
   }
  }
  if(name==='slice'){
   ...
   await page.screenshot({path:'evidence/g2/slice-cutaway.png'});
   await page.setViewportSize({width:1366,height:768});await page.screenshot({path:'evidence/g2/slice-1366.png'});
   await writeFile('evidence/g2/snapshot.json',JSON.stringify(await page.evaluate(()=>window.__m0.snapshot()),null,2));
  }
  await page.close();
 }
 await writeFile('evidence/g2/capture-errors.json',JSON.stringify(errors));
 await writeFile('evidence/g2/definition-comparison.json',JSON.stringify({controlEqualsSlice:JSON.stringify(definitions.control)===JSON.stringify(definitions.slice),m4EqualsSlice:JSON.stringify(definitions.m4)===JSON.stringify(definitions.slice)},null,2));
