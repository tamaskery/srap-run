...
const output='evidence/g6/performance';
await mkdir(output,{recursive:true});
const baseline=await preview({configFile:false,build:{outDir:'evidence/g6/baseline-dist'},preview:{host:'127.0.0.1',port:4185,strictPort:true}});
const candidate=await preview({configFile:false,build:{outDir:'dist'},preview:{host:'127.0.0.1',port:4187,strictPort:true}});
...
try{
 for(const [label,port] of [['g5-1',4185],['g6-1',4187],['g5-2',4185],['g6-2',4187]]){
  const start=performance.now();
  await page.goto(`http://127.0.0.1:${port}/?debug&scene=square`);
  await page.waitForFunction(()=>window.__m0);startup.push({label,ms:performance.now()-start});
  ...
  for(const state of ['idle','walk','sprint','court']){
   ...
   await writeFile(`${output}/matched.json`,JSON.stringify({browser:browser.version(),headless:true,startup,results},null,2));
   console.log(JSON.stringify({label,state,...report.metrics}));
  }
 }
}finally{await browser.close();baseline.httpServer.close();candidate.httpServer.close();}
