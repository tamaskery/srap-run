...
try{
 for(const [label,port] of [['baseline-1',4185],['slice-1',4187],['baseline-2',4185],['slice-2',4187]]){
  ...
  for(const state of (process.env.G2_PHASES?.split(',')??['idle','walk','sprint','court'])){
   ...
   await writeFile(process.env.G2_OUTPUT??'evidence/g2/performance.json',JSON.stringify({browser:browser.version(),headless:true,startup,results},null,2));
