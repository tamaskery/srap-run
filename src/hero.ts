...
export class Hero {
 ...
 private elapsed=0;
 private lastTime=0;
 private state='hero_idle';
 private constructor(readonly root:Mesh,private readonly clips:Map<string,AnimationGroup>){
  this.select('hero_idle');
 }
 static async create(scene:Scene){
  const asset=await LoadAssetContainerAsync(`${import.meta.env.BASE_URL}assets/hero/hero.glb`,scene);
  const clips=new Map(asset.animationGroups.map(g=>[g.name,g]));
  for(const name of ['hero_idle','hero_walk','hero_sprint'])if(!clips.has(name)){
   asset.dispose();throw new Error(`Hero is missing ${name}`);
  }
  asset.addAllToScene();
  const root=new Mesh('player',scene);root.isPickable=false;
  ...
  const marker=MeshBuilder.CreateTorus('player:foot-ring',{diameter:1.3,thickness:.055,tessellation:24},scene);
  marker.parent=root;marker.position.y=-.86;marker.isPickable=false;
  const material=new StandardMaterial('hero:marker',scene);material.diffuseColor=Color3.FromHexString('#f3df98');material.specularColor=Color3.Black();marker.material=material;
  // All resources belong to this scene; no cross-replay cache or observers.
  return new Hero(root,clips);
 }
 ...
 update(before:Vec,after:Vec,time:number,active:boolean,sprinting:boolean){
  ...
  const state=moving?(sprinting?'hero_sprint':'hero_walk'):'hero_idle';
